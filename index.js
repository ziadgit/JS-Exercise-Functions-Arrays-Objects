...
/* referenced sort solution from https://flaviocopes.com/how-to-sort-array-of-objects-by-property-javascript/ */
function sortCarInventory(inv) {
  inv.sort((a, b) => (a.car_model > b.car_model) ? 1 : -1);
  return inv
}
...
function getModelYears(inv) {
  const years = inv.map(c => c.car_year);
  return years;
}
...
function getOlderCars(inv, max) {
  const cars = inv.filter(c => c.car_year <= max);
  return cars;
}
...
function getGermanCars(inv) {
  ...
  const cars = inv.filter(c => german.includes(c.car_make));
  return cars;
}
